...
export const Cart = () => {
  ...
  const taxes = total * 0.005;
  const shipping = total * 0.001;
  const net = total - taxes - shipping;
  const [promoCode, setPromoCode] = useState();
  const promo = promoCode ? total * 0.1 : 0;
  ...
  if (!carts.length)
    return (
      <div className="h-[49vh] flex justify-center items-center text-4xl">
        Cart is Empty
      </div>
    );
  // notworking
                ...
                <tbody>
                  {carts?.map((cart) => {
                    return (
                      <tr
                        key={cart.id}
                        className="flex items-center flex-col sm:table-row"
                      >
                        <td className="py-4">
                          <div className="flex items-center flex-col sm:flex-row">
                            <img
                              ...
                              alt={cart?.title}
                            />
                            <div className="flex-col items-center sm:items-start  flex">
                              <p className="font-semibold">{cart?.title}</p>
                              <p className="hidden sm:block">
                                {cart?.category}
                              </p>
                              <button
                                className="text-start border-2 rounded  text-white bg-red-500 hover:text-red-500  hover:bg-white"
          ...
          <div className="md:w-1/3">
            <div className="bg-white rounded-lg shadow-md p-6">
              ...
              <div className="flex lg:flex-row md:flex-col justify-between mb-2">
                ...
                <input
                  type="text"
                  className="outline-black border-2 lg:w-1/2 md:w-full"
                  placeholder="XXXX"
                  onChange={handlePromoCode}
                />
              </div>

              <button className="bg-red-500 text-white py-2 px-4 rounded-lg mt-4 lg:w-1/2 md:w-full">
                Apply
              </button>
              <hr className="my-2" />
              <div className="flex justify-between mb-2">
